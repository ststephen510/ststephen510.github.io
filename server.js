...
// Company database with career page URLs
const COMPANIES = [
    ...
    try {
        ...
        if (!XAI_API_KEY) {
            return res.status(500).json({ 
                error: 'xAI API key not configured. Please add XAI_API_KEY to your .env file' 
            });
        }

        // Call xAI Grok API to find and match jobs
        const jobs = await findJobsWithGrok(jobTitle, specialization, region);
...
// Function to use Grok API for job matching
...
app.listen(PORT, () => {
    console.log(`\n🚀 arcticulab.io server running on http://localhost:${PORT}`);
    console.log(`📊 Tracking ${COMPANIES.length} companies`);
    console.log(`🤖 xAI API: ${XAI_API_KEY ? '✓ Configured' : '✗ Not configured'}`);
